...
describe Company do
  ...
  describe 'no_contact_info? method' do
    ...
    it 'should exist' do
      @company.should respond_to :no_contact_info?
    end
    
    it "should be true for companies without any contact information" do
      no_contact_info_company = Company.new @attr.merge :url => '', :email => '', :phone => ''
      no_contact_info_company.no_contact_info?.should be_true
    end
    
    it "should be false for companies with one or more contact methods (url, email or phone)" do
      company = Company.new @attr
      company.no_contact_info?.should be_false
      company = Company.new @attr.merge :url => ''
      company.no_contact_info?.should be_false
      company = Company.new @attr.merge :email => ''
      company.no_contact_info?.should be_false
      company = Company.new @attr.merge :phone => ''
      company.no_contact_info?.should be_false
      company = Company.new @attr.merge :url => '', :email => ''
      company.no_contact_info?.should be_false
      company = Company.new @attr.merge :phone => '', :email => ''
      company.no_contact_info?.should be_false
      company = Company.new @attr.merge :url => '', :phone => ''
      company.no_contact_info?.should be_false
    end
  end
end
